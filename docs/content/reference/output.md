...
describes what users get. Pick a format with `-o`, or let blockchain-info choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
blockchain-info <command> -o table   # aligned columns for reading
blockchain-info <command> -o jsonl   # one JSON object per line, for piping
blockchain-info <command> -o json    # a single JSON array
blockchain-info <command> -o csv     # spreadsheet friendly
blockchain-info <command> -o tsv     # tab-separated
blockchain-info <command> -o url     # just the URL column
blockchain-info <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
blockchain-info <command> --fields id,title,url
```
...
```bash
blockchain-info <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
blockchain-info <command>            # a table, because this is a terminal
blockchain-info <command> | wc -l    # JSONL, because this is a pipe
```
